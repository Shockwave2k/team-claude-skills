...
- `backend/neolink-gatekeeper-expert` — project-specific: house conventions, tenant wiring, auth flavours, KMS/Vault, RSA/JWT/argon2 patterns, service-account signing for the `neolink-gatekeeper` Go service. Paired with the `neolink-gatekeeper-expert` agent. Auto-attaches on gatekeeper-related prompts.
...
- `shared/feature-outcome` — Phase 3 of `/feature`. Diffs the branch, writes `.claude/features/<slug>/outcome.md`, patches the affected `.claude/rules/*.md` files so the brain reflects new public APIs, schemas, and connection points. Also runs a stale-outcome housekeeping pass.
- `shared/project-expert-generator` — after `codebase-scan` has built the brain, distills it into a compact project-local skill (`.claude/skills/<slug>-expert/SKILL.md`) and opus-backed expert agent (`.claude/agents/<slug>-expert.md`). Output is trigger-phrase-rich so the skill auto-attaches on that project's tasks. Requires the generated files to be un-ignored in the project's `.gitignore` (skill prints the exact lines).
...
- `agents/backend/neolink-gatekeeper-expert.md` — deep specialist for `neolink-gatekeeper`: shared identity / auth / IAM service for Neolink and Hive. Go + Gin + MSSQL/Mongo/Redis + KMS/Vault. Generated by `project-expert-generator`; canonical source of truth lives here, installable into adjacent projects via the skill-manager or `--agent=neolink-gatekeeper-expert`.
...
**On project-specific experts**: entries like `neolink-gatekeeper-expert` (skill + agent) are generated by `project-expert-generator` and live natively in the owning project's `.claude/`. They get copied UPSTREAM here when they're useful to adjacent projects. Canonical source of truth then lives in this repo — the project's local file should eventually become a symlink to this one for sync. When the underlying codebase evolves meaningfully, re-run `codebase-scan` + `project-expert-generator` in the project, then manually re-sync into this repo.
